...
export function InvoiceList({ initialInvoices }: { initialInvoices: Invoice[] }) {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      window.dispatchEvent(new Event('trash-updated')); // To update trash count in sidebar
    } catch (err) {
  ...
  return (
    <div>
      ...
      {error && (
        <div className="flex items-center p-4 mb-4 text-sm text-red-400 bg-red-900/20 rounded-lg">
      ...
      {paginatedInvoices.length === 0 && !isLoading ? (
        <div className="glass-panel p-8 text-center text-white/50">
          ...
          <p className="mt-2">Try adjusting your search/filters or create a new invoice.</p>
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-white/10">
              {paginatedInvoices.map((invoice) => (
                <tr key={invoice._id?.toString()}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-white/70">{invoice.client?.name || 'N/A'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-white/70">{invoice.metadata?.issueDate || 'N/A'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-white">{invoice.summary?.total != null ? `$${invoice.summary.total.toFixed(2)}` : '$0.00'}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    ...
                    >
                      {deletingId === invoice._id?.toString() ? <Loader2 className="w-5 h-5 animate-spin inline-block" /> : <Trash2 className="w-5 h-5 inline-block" />}
                    </button>
                  </td>
                </tr>
              ))}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-6">
          <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-4 py-2 text-sm rounded-md bg-white/10 text-white disabled:opacity-50 hover:bg-white/20">
            Previous
          </button>
          <span className="text-sm text-white/50">Page {currentPage} of {totalPages}</span>
          <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-4 py-2 text-sm rounded-md bg-white/10 text-white disabled:opacity-50 hover:bg-white/20">
            Next
          </button>
        </div>
      )}
