...
export default function ResetRequestPage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Always show the "check your email" screen (prevents user enumeration)
      setSubmitted(true)

      // Dev only: the API echoes back the link so we can click it without an SMTP server
      if (data.devResetUrl) {
  ...
  if (submitted) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="glass-panel p-8 w-full max-w-md space-y-6 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-white/50 mt-2">
              If <span className="text-white/70">{email}</span> is registered, we&apos;ve sent a
              password reset link. It expires in 30 minutes.
            </p>
          </div>

          {/* Development helper — NOT shown in production */}
          {devResetUrl && (
            <div className="text-left bg-amber-500/10 border border-amber-500/30 rounded-lg p-4 space-y-2">
          ...
          <Link
            href="/auth/signin"
            className="inline-flex items-center gap-1.5 text-sm text-crystal-400 hover:text-crystal-300 transition-colors"
          >
